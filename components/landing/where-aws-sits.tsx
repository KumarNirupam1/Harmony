...
export function WhereAWSits() {
  return (
    <section id="stack" className="mx-auto max-w-6xl px-6 pb-24">
      ...
      <h2 className="font-display mt-3 text-center text-4xl tracking-tight">Where AWS sits</h2>
      <p className="mx-auto mt-3 max-w-2xl text-center text-muted">
        A static web app on CloudFront + S3. A PyTorch engine inside a Lambda
        container behind API Gateway. Mission history in DynamoDB. Pay nothing
        until a cleanup plan actually runs.
      </p>
      <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        {["API Gateway", "Lambda", "S3 + CloudFront", "DynamoDB", "PyTorch", "Next.js 16"].map(
          (t) => (
            <div
              key={t}
              className="rounded-2xl border border-border bg-panel px-4 py-4 text-center text-xs font-medium"
            >
              {t}
            </div>
          ),
